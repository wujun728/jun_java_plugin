	...
	$.extend(wform,{
		...
		message : {
            ...
            password: '密码不符合规范',
				...
				'number': '该字段只能为数字',
				'letterAndNum':'只允许英文、数字、连接线',
				'legalStr':'不能输入特殊字符',
                'commaAndNum':'只允许输入逗号和数字'
			},
		validate : function(set) {
			...
			validateItem : function(id, a, v) {
				...
				if (a['server']) {
					...
					var otherParams=a['server']['otherParams'];
					var data={};
					data[pname]=v;
					
					if(otherParams){
						if(!(otherParams instanceof Array)){
							otherParams=new Array(otherParams);
						}
						
						for(var i=0;i<otherParams.length;i++){
							var op=otherParams[i];
							var pname2=op['pname'];
							if(pname2==null){
								pname2=op['id'];
							}
					...
					var remoteRes=form.validator.requestValidate(url,data);
					
					if (!remoteRes.flag) {
						if(remoteRes.message!=null && remoteRes.message!=''){
							msg=remoteRes.message;
						}
			...
			validator : {
						...
						requestValidate: function(url,data) {
							var remoteRes={flag:false};
							$.ajax({
								url: url,
								type: "POST",
								cache: false,
								...
								success: function(data, textStatus) {
									if(data!=null){
										remoteRes.flag = data['success'];
										remoteRes.message=data['message'];
									}
								}
							});
							return remoteRes;
