...
public class TestRedisZDD
{
    ...
    public static void main(String[] args) throws InterruptedException
    {
        ...
        String szKey = "fzjh_sct";
        
        
        RedisSharedMasterSlaveSentinelUtil.zadd(szKey, 1, "sid1");
        RedisSharedMasterSlaveSentinelUtil.zadd(szKey, 6, "sid2");
        RedisSharedMasterSlaveSentinelUtil.zadd(szKey, 2, "sid3");
        RedisSharedMasterSlaveSentinelUtil.zadd(szKey, 9, "sid4");
        RedisSharedMasterSlaveSentinelUtil.zadd(szKey, 0, "sid5");
        
//        double oldConnection = RedisSharedMasterSlaveSentinelUtil.zscore(szKey, "sid");
//        RedisSharedMasterSlaveSentinelUtil.zadd(szKey, oldConnection+1, "sid"); 
        ...
        RedisSharedMasterSlaveSentinelUtil.del(szKey);
    }

    
    public static void getDescKey(String i,String szKey)
    {
        Set<String> set = RedisSharedMasterSlaveSentinelUtil.zrange(szKey, 0, 0);
        if(null != set && set.size() > 0)
        {
            System.out.println("size"+set.size());
            String key = set.iterator().next();
            double oldConnection = RedisSharedMasterSlaveSentinelUtil.zscore(szKey, key);
            double connection = oldConnection+1;
            RedisSharedMasterSlaveSentinelUtil.zadd(szKey, connection, key);
            System.out.println(i+","+oldConnection+ "=" +key);
