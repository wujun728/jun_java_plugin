package com.opensource.nredis.proxy.monitor.object.utils;
...
import com.opensource.nredis.proxy.monitor.file.utils.ObjectUtil;
			...
			for (Field field : fields) {
				...
				if (mapField.containsKey(field.getName())) {
					...
					//Collection 类型
						...
						Collection listObject = ObjectUtil.cast(newInstance(fieldType));
						listObject.addAll((Collection) ObjectUtil.cast(value));
						value = listObject;
					}
					//如果没有匹配到类型且参数是 Map 类型,就判定是复杂类型,递归调用方法
					else if(isImpByInterface(value.getClass(),Map.class)){
						value = getObjectFromMap(fieldType, (Map<String, Object>) ObjectUtil.cast(value));
					}
