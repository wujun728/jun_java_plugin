...
public class MyPrinter2 {
	...
	static final String SPLIT_LINE = "=";// 分割�?
	...
	/**
	 * Arrays有toString方法，但是对象内容太多，在一行显�?还有就是没有显示index信息
	 */
	...
	/**
	 * map的value是简单类型的，复制Map.toString，我给它加了换行10个换�?
	 * 
	 * @param map
	 * @return
	 */
	...
	private static String beanToStr(Object object, boolean recursion) {
		...
		for (int i = 0;; i++) {
			...
			field.setAccessible(true);// 设置些属性是可以访问�?
			String name = field.getName();// 取得field的名�?
			if (name.equals("serialVersionUID"))
				continue;
			try {
				Object value = field.get(object);// 得到此属性的�?
				if (isSimpleType(value) || !recursion)
	...
	public static Field[] sortFieldByType(Field[] fields) {
		for (int i = 0; i < fields.length; i++) {
			if (isSimpleType(fields[i].getType()))
				continue;// fields[i]是简单类型不�?
			// fields[i]是复杂类�?
			// int j = i+1，从fields[i]之后�?��比较
			for (int j = i + 1; j < fields.length; j++) {
				Field fieldTmp = null;
				if (isSimpleType(fields[j].getType())) {// 与后面的第一个简单类型交�?
					fieldTmp = fields[i];
					fields[i] = fields[j];
					fields[j] = fieldTmp;
					break; // 后面的循环，是没有意义de
	...
	/**
	 * 这个方法是�?归方法，并且并多个地方调用，考虑到循环引用和显示格式�?boolean recursion取得确保递归可以被终止�?
	 * 
	 * @param object
	 * @param recursion
	 *            是否�?��更深�?��显示
