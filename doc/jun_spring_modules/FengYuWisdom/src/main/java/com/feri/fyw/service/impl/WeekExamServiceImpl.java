...
import com.feri.fyw.dao.WeekExamDao;
import com.feri.fyw.entity.WeekExam;
import com.feri.fyw.listener.WeekExamReadListener;
...
public class WeekExamServiceImpl implements WeekExamService {
    ...
    public R saveBatch(MultipartFile file) {
        if(file.isEmpty()){
            try {
                List<WeekExam> list= EasyExcel.read(file.getInputStream(),WeekExam.class,new WeekExamReadListener()).doReadAllSync();
    ...
    public PageBean queryAll(int page, int limit) {
        ...
        PageInfo<WeekExam> pageInfo=new PageInfo<>(dao.selectAll());
