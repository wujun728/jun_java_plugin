  ...
  <div>
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item>
        ...
        <el-button icon="el-icon-s-promotion" plain type="info" size="mini" @click="jumpFormBuild">跳转表单构建</el-button>
        <el-button icon="el-icon-s-promotion" plain type="info" size="mini" @click="jumpTextConvert">文本格式转换</el-button>
      </el-form-item>
    </el-form>
    ...
    <div style="padding:5px 10px 15px 10px">
      <el-button type="primary" plain icon="el-icon-download" size="mini" @click="handleGenTable">生成</el-button>
...
<script>
import { listTable, previewTable, delTable, genCode, synchDb } from "@/api/gen";
  ...
  methods: {
    ...
    handleGenTable(row) {
      ...
      if (row.genType === "1") {
        ...
        this.$download.zip("/gen/batchGenCode?tables=" + tableNames, "ruoyi.zip");
      }
    },
    /** 同步数据库操作 */
    handleSynchDb(row) {
    ...
    handleEditTable(row) {
      ...
      this.$tab.openPage("修改[" + tableName + "]生成配置", '/gen-edit/index/' + tableId, params);
    },
    /** 删除按钮操作 */
    handleDelete(row) {
