...
//	}
	...
	private static void checkDbProNull() {
		if (ObjUtil.isEmpty(MAIN)) {
			throw new ActiveRecordException("请调用Db.init初始化数据源(仅需初始化一次即可)");
		}
	}
	...
	public static  <T> List<JSONObject> queryList(String sql) {		// return  List<object[]> or List<object>
		checkDbProNull();
		return MAIN.queryJSON(sql);
	}
	public static  <T> List<JSONObject> queryJSON(String sql) {		// return  List<object[]> or List<object>
		checkDbProNull();
		return MAIN.queryJSON(sql);
	}
	public static  <T> List<JSONObject> queryJSON(String sql, Object... paras) {
		checkDbProNull();
		return MAIN.queryJSON(sql,paras);
	}
