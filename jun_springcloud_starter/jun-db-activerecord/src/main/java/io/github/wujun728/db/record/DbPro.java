...
//	}
	...
	public Object executeSqlXml(String sqlXml, Map params) throws SQLException {
		return SqlXmlUtil.executeSql(config.getDataSource().getConnection(), sqlXml, params, true);
	}

	public int updateSqlXml(String sqlXml, Map params) throws SQLException {
		return SqlXmlUtil.update(config.getDataSource().getConnection(), sqlXml, params);
	}

	public List<Map<String, Object>> querySqlXml(String sqlXml, Map params) throws SQLException {
		return SqlXmlUtil.query(config.getDataSource().getConnection(), sqlXml, params);
	}
	...
	public <T> List<JSONObject> queryList(String sql) {		// return  List<object[]> or List<object>
		return queryJSON(sql, DbKit.NULL_PARA_ARRAY);
	}
	public <T> List<JSONObject> queryJSON(String sql) {		// return  List<object[]> or List<object>
		return queryJSON(sql, DbKit.NULL_PARA_ARRAY);
	}
	public <T> List<JSONObject> queryList(String sql, Object... paras) {
		return queryJSON(sql, paras);
	}
	public <T> List<JSONObject> queryJSON(String sql, Object... paras) {
		Connection conn = null;
		try {
			conn = config.getConnection();
			return queryJSON(config, conn, sql, paras);
		} catch (Exception e) {
			throw new ActiveRecordException(e);
		} finally {
			config.close(conn);
	...
	protected <T> List<JSONObject> queryJSON(Config config, Connection conn, String sql, Object... paras) throws SQLException {
		//List result = new ArrayList();
		try (PreparedStatement pst = conn.prepareStatement(sql)) {
			config.dialect.fillStatement(pst, paras);
			ResultSet rs = pst.executeQuery();
	...
	public List<Record> findByColumnValueRecords(String tableName, String columnNames, Object... columnValues) {
		...
		String sql = config.getDialect().forDbFindById(tableName, pKeys);

		List<Record> resultMap = find(sql, columnValues);
		return resultMap;
	}
	...
	public <T> List findByWhereSqlForBean(Class clazz, String whereSql, Object... columnValues) {
		...
		List<Record> resultMap = find(sql, columnValues);
		return RecordUtil.recordToListBean(resultMap, clazz);
	}
	...
	public Page<Record> paginate(Integer pageNumber, Integer limit, String select, String from/*, Map<String, Object> params*/) {
		...
		List<JSONObject> results = queryJSON(SqlUtil.getSelect(sqlStr, pageNumber, limit), null);
...
//    }
	...
	public <T> T findBeanById(Class<T> clazz, Object... idValue) {
		String tableName = SqlUtil.getTableName(clazz);
		String primaryKeyStr = config.getDialect().getDefaultPrimaryKey();// getPkNames(tableName);
	...
	public <T> T findBeanByIds(Class beanClass, String primaryKeys, Object... idValue) {
		...
		if (primaryKeys == null || StrUtil.isEmpty(primaryKeyStr)) {
			primaryKeyStr = config.getDialect().getDefaultPrimaryKey();//getPkNames(tableName);
		}
		Record record = findByIds(tableName, primaryKeyStr, idValue);
		if (record == null) {
			return null;
	...
	public <T> Page findBeanPages(Class beanClass, int page, int rows, Map<String, Object> params) {
		...
		//SqlContext sqlContext1 = SqlUtil.getSelect(new StringBuilder(sql), params);
		//SqlContext sqlContext = SqlUtil.getSelect(sql, page, rows);
		List<JSONObject> listData = queryList(SqlUtil.getSelect(sql, page, rows), params);
		//List<Map<String, Object>> listData = queryList(sqlContext);
		//RecordUtil.mapToBeans(listData, beanClass)
		pageVo.setList(listData);
	...
	public Page queryBeanPage(Class beanClass, int page, int rows, Map<String, Object> params) {
		...
		Page pageVo = new Page();
		String sql = SqlUtil.getSelectSQl(beanClass);
		SqlContext sqlContext = SqlUtil.getSelect(new StringBuilder(SqlUtil.getSelect(sql, page, rows)), params);
		//List<Map<String, Object>> listData = queryList(sqlContext.getSql(), sqlContext.getParams());
		List<JSONObject> listData = queryList(sqlContext.getSql(), sqlContext.getParams());
