package com.jun.plugin.leetcode.algorithm.no0212;
 ...
 * 字典树 + DFS
 *
 * 思路是，将 words 建立字典树，然后对 board 进行 dfs 检索，比较 words 字典树。
 */
public class Solution {
    ...
    private void dfs(List<String> results, String str, int maxLength, TrieNode current, char[][] board, int i, int j) {
        ...
        // 标记为 # ，代表已经访问
        str += ch; // 注意，这里可以进一步优化，避免字符串拼接。TrieNode 里，存储 word 。在符合时候，直接使用。具体可以参考 https://leetcode-cn.com/problems/word-search-ii/solution/java-zi-dian-shu-shi-jian-17miao-ji-bai-98kong-jia/ 的代码。
        board[i][j] = '#';
